...
axes(handles.axes1);
imshow(I2);title('Orginal Görüntü');
ss = ones(300,400);
axes(handles.axes2);
imshow(ss);
axes(handles.axes3);
...
axes(handles.axes2);
imshow(I5);title(' Konstratý Arttýrýlmýþ Görüntü ');
handles.ImgData2 = I4;
guidata(hObject,handles);
...
figure,subplot(2,3,2);imshow(I);title('Orjinal Görüntü'); subplot(2,3,4);imshow(segmented_images{1});title('Segmente Edilmiþ 1.Görüntü'); subplot(2,3,5);imshow(segmented_images{2});title('Segmente Edilmiþ 2.Görüntü');
subplot(2,3,6);imshow(segmented_images{3});title('Segmente Edilmiþ 3.Görüntü');
...
if Affected_Area < 0.1
    Affected_Area = Affected_Area*100;
end
...
axes(handles.axes3);
imshow(I7);title('Segmente Edilmiþ Görüntü');
...
itr = 500;
hWaitBar = waitbar(0,'500 iterasyondan sonra Linear Kernel Doðruluk Deðeri:');
for i = 1:itr
data = Train_Feat;
